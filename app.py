...
def require_auth(f):
    ...
    def decorated(*args, **kwargs):
        if not SUPABASE_JWT_SECRET:
            # Fail closed: a misconfigured deploy must not silently accept
            # unauthenticated traffic. (See RELI-08 in audit.)
            return jsonify(error='Server misconfigured: missing JWT secret'), 500
